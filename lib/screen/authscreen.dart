...
class _AuthScreenState extends State<AuthScreen> {
  ...
  // If the current page URL IS the sign-in link the user clicked from their
  // inbox, finish the sign-in automatically. Call this once on startup —
  // it's a no-op if the URL isn't a sign-in link.
  Future<void> _completeEmailLinkSignInIfPresent() async {
    ...
    final email = prefs.getString(_kPendingEmailKey);

    if (email == null) {
      // Link opened in a different browser/device than it was requested
      // from — ask the user to re-enter their email to confirm identity.
      if (!mounted) return;
      final confirmed = await _promptForEmail();
      if (confirmed == null) return;
      await _finishEmailLinkSignIn(confirmed, currentUrl);
      return;
    }
  ...
  Future<void> _sendEmailLink() async {
    ...
    try {
      final actionCodeSettings = ActionCodeSettings(
        // Must be an authorized domain (see Firebase Console → Auth →
        // Settings → Authorized domains). This brings the user back to
        // this same page after clicking the emailed link.
...
/// Minimal inline Google "G" glyph so this file has zero extra asset/icon
